...
#### Makefile команды
Для удобства разработки доступны команды через Makefile:

```bash
# Установка окружения для разработки
make dev-install

# Проверка и исправление качества кода
make quality
...
Все инструменты настроены через единый файл конфигурации:
- **Ruff**: линтинг (flake8, isort, pycodestyle, etc.) и форматирование
- **MyPy**: строгая проверка типов с поддержкой внешних библиотек
- **Pytest**: настройки для автоматических тестов
