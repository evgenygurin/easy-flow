...
**Реализованные:**
- **Yandex Alice** ✅ - российский голосовой помощник с полной интеграцией
...
```http
# Обработка webhook от голосового помощника
POST /api/v1/voice/webhook/{platform}

# Специальный endpoint для Yandex Alice
POST /api/v1/voice/alice

# Получение поддерживаемых платформ
GET /api/v1/voice/platforms

# Health check голосовых интеграций
GET /api/v1/voice/health
```

##### Пример интеграции с Yandex Alice

**Webhook настройка:**
```bash
# Переменные для Yandex Alice
ALICE_WEBHOOK_URL=https://your-domain.com/api/v1/voice/alice
ALICE_OAUTH_TOKEN=your-alice-oauth-token
ALICE_WEBHOOK_SECRET=your-webhook-secret  # опционально
```

**Обработка голосовых запросов:**
```python
# Автоматическая обработка Alice запросов
@app.post("/api/v1/voice/alice")
async def alice_webhook(request: Request):
    payload = await request.json()
    
    # Извлечение намерений и сущностей
    intent = payload["request"]["nlu"]["intents"]
    user_text = payload["request"]["original_utterance"]
...
##### Поддерживаемые функции в Alice

- ✅ **Проверка статуса заказа** - "Алиса, где мой заказ 12345?"
- ✅ **Поиск товаров** - "Алиса, найди смартфоны Samsung"
- ✅ **Добавление в корзину** - "Алиса, добавь в корзину"
- ✅ **Помощь и навигация** - "Алиса, что ты умеешь?"
- ✅ **Управление сессией** - сохранение контекста между запросами
- ✅ **Rich Cards** - для устройств с экраном
- ✅ **Обработка ошибок** - graceful degradation

##### Примеры диалогов с Alice

**Проверка заказа:**
```
Пользователь: "Алиса, где мой заказ?"
Alice: "Для проверки статуса заказа назовите номер заказа."

Пользователь: "Двенадцать тысяч триста сорок пять"
Alice: "Заказ 12345 обрабатывается. Ожидаемая дата доставки: завтра."
```

**Поиск товаров:**
```
Пользователь: "Алиса, найди телефон"
Alice: "Нашел несколько товаров по запросу 'телефон'. Хотите добавить что-то в корзину?"

Пользователь: "Да, первый"
Alice: "Товар добавлен в корзину. Хотите оформить заказ или продолжить покупки?"
```
...
```
app/
├── adapters/voice/              # Голосовые адаптеры
│   ├── base.py                  # Базовый класс
│   ├── yandex_alice.py         # ✅ Yandex Alice
│   ├── amazon_alexa.py         # 🔄 Amazon Alexa  
│   ├── google_assistant.py     # 🔄 Google Assistant
│   └── apple_siri.py           # 🔄 Apple Siri
├── services/voice_service.py    # Бизнес-логика голоса
├── models/voice.py             # Модели голосовых сообщений
└── api/
    ├── controllers/voice_controller.py  # HTTP контроллер
    └── routes/voice.py         # API маршруты
```
...
class VoiceMessage(BaseModel):
    platform: VoicePlatform  # yandex_alice, alexa, google, siri
    session_id: str          # ID голосовой сессии
    ...
    # Rich content для устройств с экраном
    card: VoiceCard | None          # Карточка с контентом
    directives: list[VoiceDirective] # Специфичные команды платформы
...
- [x] **Голосовые ассистенты** ✨ - Yandex Alice полностью реализована
- [ ] Дополнительные голосовые платформы (Alexa, Google Assistant, Siri)
